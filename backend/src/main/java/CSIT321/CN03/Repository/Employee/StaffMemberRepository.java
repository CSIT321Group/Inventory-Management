...
public interface StaffMemberRepository extends JpaRepository<StaffMember, Long> {
    ...
    @Query("SELECT s FROM StaffMember s WHERE " +
            "LOWER(s.userName) LIKE LOWER(CONCAT('%', ?1, '%')) OR " +
            "CAST(s.id AS string) LIKE ?1 OR " +
            "LOWER(CONCAT(s.first_name, ' ', s.last_name)) LIKE LOWER(CONCAT('%', ?1, '%')) OR " +
            "LOWER(s.first_name) LIKE LOWER(CONCAT('%', ?1, '%')) OR " +
            "LOWER(s.last_name) LIKE LOWER(CONCAT('%', ?1, '%'))")
